function [SDR, SIR, SAR, SSperf] = framework_bach10d(param, script_name, script_fullpath)
diary off;
clc;
...
send_mail_at_error = 0;
send_mail_at_finish= 0;
...
if send_mail_at_start
    title = ['Start: MPE-ISS score-informed CSC at ', IPaddress, ' ', script_name ' starts'];
...
%     SSperf = main_bach10_separation_v3b(param);
    SSperf = main_bach10_separation_v3d(param);
    [mSDR, mSIR, mSAR, SDR, SIR, SAR] = perfStatistic_SS(SSperf);
    draw_good();
catch err
    diary off
    if send_mail_at_error
        title = ['Error: MPE-ISS score-informed CSC at ' IPaddress ' ' script_name];
...
if send_mail_at_finish
    % title
    title = ['Finish: MPE-ISS score-informed CSC at ' IPaddress ' ' script_name];
    % contents

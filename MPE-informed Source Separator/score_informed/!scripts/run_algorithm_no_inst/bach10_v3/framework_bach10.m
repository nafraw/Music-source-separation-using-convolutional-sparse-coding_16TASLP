...
if send_mail_at_start
    title = ['Start: MPE-ISS score-informed w/o instrument CSC at ', IPaddress, ' ', script_name ' starts'];
...
try
    param
    param.exp_name
    SSperf = main_bach10_separation_no_inst_v3(param);
    [mSDR, mSIR, mSAR, SDR, SIR, SAR] = perfStatistic_SS(SSperf);
    draw_good();
catch err
    diary off
    if send_mail_at_error
        title = ['Error: MPE-ISS score-informed w/o instrument CSC at ' IPaddress ' ' script_name];
...
if send_mail_at_finish
    % title
    title = ['Finish: MPE-ISS score-informed w/o instrument CSC at ' IPaddress ' ' script_name];
    % contents

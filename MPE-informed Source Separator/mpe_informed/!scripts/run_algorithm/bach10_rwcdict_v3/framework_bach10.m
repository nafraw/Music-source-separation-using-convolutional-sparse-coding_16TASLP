...
if send_mail_at_start
    title = ['Start: MPE-ISS CSC at ', IPaddress, ' ', script_name ' starts'];
...
try
    param
    param.exp_name
    SSperf = main_bach10_separation_v3(param);
    [mSDR, mSIR, mSAR, SDR, SIR, SAR] = perfStatistic_SS(SSperf);
    draw_good();
catch err
    diary off
    if send_mail_at_error
        title = ['Error: MPE-ISS CSC at ' IPaddress ' ' script_name];
...
if send_mail_at_finish
    % title
    title = ['Finish: MPE-ISS CSC at ' IPaddress ' ' script_name];
    % contents
    ...
    mail_contents{end+1} = sprintf('--------------------------');
    mail_contents{end+1} = sprintf('Lambda: %f', param.algorithm.lambda);
    mail_contents{end+1} = sprintf('Iter#: %d', param.algorithm.opt.MaxMainIter);
    mail_contents{end+1} = sprintf('gamma: %f', param.algorithm.opt.gamma);
    mail_contents{end+1} = sprintf('RelaxParam: %f', param.algorithm.opt.RelaxParam);
    mail_contents{end+1} = sprintf('L1Weight: %f', param.algorithm.opt.L1Weight);
    % attachments
        time = clock; % cuurent time
        % the name of attachment is given by a given name and time and date.

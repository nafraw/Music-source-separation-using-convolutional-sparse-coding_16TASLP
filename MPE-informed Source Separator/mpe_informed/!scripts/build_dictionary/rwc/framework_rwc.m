function framework_rwc(param, script_name, script_fullpath)
diary off;
clc;
...
if send_mail_at_start
    title = ['Start: MPE-ISS Build Dictionary at ', IPaddress, ' ', script_name ' starts'];
...
diary(diary_name);
try
    main_build_dict_bach10(param);
catch err
    diary off
    if send_mail_at_error
        title = ['Error: MPE-ISS Build Dictionary at ' IPaddress ' ' script_name];
...
if send_mail_at_finish
    % title
    title = ['Finish: MPE-ISS Build Dictionary at ' IPaddress ' ' script_name];
    % contents

...
param.midname_train_corpus = '_use_aux_';

%% dictionary training (building) process
param.nmf = 1;
param.dict_train.sampling_rate = 44100;
param.dict_train.target_dataset = 'Bach10_single_note';
...
for fold = 1:length(param.train_fold)
    for inst = 1:length(param.part_name)
        %% setting training data path        
        param.path_pre_process_external_corpus{fold, inst} = ...
            [mount_path, param.rootpath_train_corpus, param.part_name{inst}, ...
            param.midname_train_corpus, num2str(param.train_fold{fold})];
...
%         clear idata;        
        ...
        param.path_dict{fold, inst} = [dict_root_path, param.dict_train.dict_type, '_', ...
            num2str(param.dict_train.num_codeword_per_pitch), 'NMF_Bach10_', ...

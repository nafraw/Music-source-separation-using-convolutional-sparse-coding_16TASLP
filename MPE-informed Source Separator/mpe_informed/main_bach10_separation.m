...
%  param.
function [SSperf] = main_bach10_separation(param)
start_max_pool;
...
param.path_mpe_result = strcat(param.path_mpe_result, filelist, '.txt');
...
display('Starting source separation...');
    ...
    for ffid = 1 : length(param.test_fold{fold})
        ...
        [maskD, partition_set] = mpe_informed_CSC(param, fid, P, D, zero_pad, dict_sid, pitch);
        %% output stage
        ...
        [R] = reconstruct_window_wise(param.path_recon{fid}, sampling_rate, D, maskD, ...
            param.path_separation{fid}, norm_ratio, ...
            partition_set, param.reconstruct ...
            );
